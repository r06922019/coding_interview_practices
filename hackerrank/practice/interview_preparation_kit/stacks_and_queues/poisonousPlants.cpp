...
int poisonousPlants(vector<int> plants) {
    list<list<int>> list_of_stacks;
    list<int> cur_list;
    for(int &plant : plants) {
        // maintain each stack's decreasing order
        if(cur_list.size() > 0 && cur_list.back() < plant) {
            list_of_stacks.push_back(cur_list); // copies
            cur_list.clear();
        }
        cur_list.push_back(plant);
    }
    list_of_stacks.push_back(cur_list);
    int days = 0;
    while(list_of_stacks.size() > 1) {
        auto it = list_of_stacks.begin();
        ++it;
        for(; it != list_of_stacks.end(); ++it) {
            // remove each stacks (instead of first one) front
            it->pop_front();
        }
        it = list_of_stacks.begin();
        while(it != list_of_stacks.end()) {
            auto next_it = it;
            ++next_it;
            if(next_it == list_of_stacks.end()) break;
            if(next_it->empty()) { // if next empty
                list_of_stacks.erase(next_it);
            }
